...
//!
//! SOM never publishes inline: it stages the `audit.domain_event` row
//! (`published_to_mq = false`) in the handler's transaction and the outbox relay
//! delivers it after commit. Unlike the provisioning worker, SOM's publisher
//! stamps `service_identity` from the context.
