...
//!
//! Subscription never publishes inline: it stages the `audit.domain_event` row
//! (`published_to_mq = false`) in the caller's transaction and the outbox relay
//! delivers it after commit. Stamps `service_identity` from the context. The
//! Python `exchange` kwarg is accepted-but-ignored at the call sites — the relay
