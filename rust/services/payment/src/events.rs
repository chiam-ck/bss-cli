...
//!
//! Payment never publishes inline and runs no relay: it stages the
//! `audit.domain_event` row (`published_to_mq = false`) in the caller's
//! transaction and returns. (The Python lifespan opens no MQ connection either —
//! the durable audit row is the record; whatever relays it lives outside payment.)
//! Stamps `service_identity`/`actor`/`channel`/`tenant` from the request context.
