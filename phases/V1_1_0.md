...
|---|---|---|
| **catalog** | `offer-definition.register`, `promo_code.register` (create saga); `offer.issue` (targeted assignment); `promo_code.show`, `offer.list` (validation / preview / entitlement reads) | Owns the money terms + the "what is this promo / who has it" questions |
...
6. **Targeted = codeless assigned offer (`offer.issue`).** Operator pre-assigns offers to chosen customers; they auto-apply at order and show on the dashboard. No typed code, no distribution channel, **no loyalty change**. (Personalized typed codes were rejected because they'd require adding `customer_id` to loyalty's `promo_code.register`.)
...
- `code TEXT` (nullable — null for codeless targeted promotions)
...
2. → loyalty offer-definition.register          → OD-xyz   (Idempotency-Key = promotion id)
...
## Flow 3 — targeted: operator pre-assigns a codeless offer (+ the simulator)

**Pairing is NOT in the campaign.** loyalty's `Campaign` has no audience field — it only groups `offer_definition_ids` behind an approval workflow. Targeting lives at the Offer level: one `offer.issue` per chosen customer. loyalty has **no bulk tool** — assignment is a per-customer loop. **That loop is the simulator.**

### The simulator — `bss promo assign` (operator-only) + `seed_targeted_campaign.py`

Lives in catalog (holds the `LoyaltyClient`, owns the promotion↔OD link). Operator picks the audience manually:

```
bss promo assign --promo SUMMER25-VIP \
   --customers CUST-001,CUST-007,CUST-042     # explicit (the "I decide" path)
   # alternatives: --from-csv vip.csv | --segment SEG-VIP | --random 20 (demo)
```

For each customer it calls `offer.issue(offer_definition_id=<from catalog.promotion>, customer_id, source={"type":"campaign", ...})`, leaving an `issued` offer on that customer. BSS records nothing extra — loyalty is the ledger, queryable via `offer.list`. `seed_targeted_campaign.py` (mirrors the existing `seed_customers.py`) produces repeatable demo data.

### Eligibility — enforced by *presence*, not rejection

A targeted offer isn't typed in; it's looked up by the authenticated `customer_id`. A non-targeted customer simply *has no offer issued to them*, so no discount applies — there is no "wrong customer" error path in the codeless model. loyalty is the hard gate (`offer.advance_to_claimed` / `offer.redeem` only work on an offer that is actually that customer's and in the right state); BSS pre-checks via `offer.list` purely for clean UX.

### Order-time flow for an assigned offer

```
issued (assigned)  ──offer.advance_to_claimed──▶  claimed  ──offer.redeem(order_ref)──▶  redeemed

COM.create_order(customer, offering):                       # no code typed
  offer.list(customer, state∈{issued,claimed})  → candidate offers
  for each, catalog.promotion by OD → applicable to this offering?
  pick best applicable (or none); compute effective; stamp order_item

COM.handle_service_order_completed:
  offer.advance_to_claimed(offer_id) if issued
  subscription.create(... effective ...)   # charge, set discount_periods_remaining
  on success → offer.redeem(offer_id, order_ref=order_id)
  on decline → offer.revoke(offer_id, ORDER_FAILED)
```
...
| catalog service | `create_promotion` (saga), `assign_targeted` (the simulator loop), `get_by_offer_definition_id`, `preview_promo`, `validate_for_order`, `list_customer_offers` (read-proxy); holds `LoyaltyClient` |
| catalog routes | TMF671 promo routes + `GET /promo/preview` + `GET /promo/customer-offers` (portal-facing reads) |
| COM | `order_item` discount cols; `create_order(discount_code=)` + assigned-offer discovery; new `check_promo_redeemable` policy; claim/advance/redeem/revoke wiring in `handle_service_order_completed` / `_failed` / `cancel_order`; discount carried in price-snapshot payload |
...
| simulator | `seed_targeted_campaign.py` at repo root (mirrors `seed_customers.py`) |
