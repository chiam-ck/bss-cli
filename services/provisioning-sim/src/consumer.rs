...
use bss_db::PgPool;
use bss_events::{resubscribe_backoff, MqChannel, INITIAL_RESUBSCRIBE_BACKOFF};
...
/// Bind the queue and drive the consume loop, re-subscribing (with backoff) when
/// the broker drops the stream. Spawned from `main`; never returns.
