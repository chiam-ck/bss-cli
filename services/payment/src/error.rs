...
//! `HTTPException(404)` renders `{detail}`. A tokenizer `ValueError` (missing
//! `customer_external_ref`, non-positive amount) or transport fault bubbles up as
//! FastAPI's default `500 {detail:"Internal Server Error"}` — reproduced by
//! [`ApiError::Internal`].
...
pub enum ApiError {
    ...
    NotFound(String),
    /// `500 {detail}` — DB/transport/tokenizer fault; not a domain outcome.
    Internal(String),
}
