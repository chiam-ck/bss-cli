...
**Context:** The operator hit "Something went wrong" creating an order for `chiamck+001@icloud.com` on number 90000001. Investigation: `portal_auth.identity` held `customer_id` values for three `chiamck+00N` identities whose `crm.customer` rows no longer existed (only 2 customers survive, both from 07-21; the three `customer.created` audit events from 07-20 are still in `audit.domain_event`). Signup's returning-customer branch calls `crm.get_kyc_status(cid)`, which returns 422 `POLICY_VIOLATION` / `customer.kyc.not_found`; the rule had no entry in `RULE_MESSAGES`, so it rendered `GENERIC_FALLBACK`. **Not a code regression** — no signup-path change caused it, and no in-tree code deletes arbitrary customers (`seed-demo --reset` matches exact `*.demo` emails; `make reset-db` would have taken `portal_auth` + `audit` too), so the CRM rows were removed out-of-band around 07-21. The defect is that the portal had no handling for the state: the identity was bricked permanently, and because reserve-at-pick runs *before* the customer lookup, every retry burned another MSISDN hold (`OO-3ea9832…` was still holding 90000001).
