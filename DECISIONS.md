...
## 2026-07-05 — v1.8.0 — Branding via a new bss-branding package + [branding] in settings.toml
**Context:** The product "face" was hardcoded everywhere: `bss-cli` name + `$`/`●` marks in both portal layouts, a phosphor palette duplicated between `portal_base.css` and `_EMAIL_*` constants in `bss-portal-auth/email.py`, the version number squatting in header title lines, and a separate `BSS_OPERATOR_NAME` env for the chat greeting. An operator deploying BSS-CLI as a real MVNO could not rebrand without forking templates.
**Decision:** New tiny workspace package `bss-branding` (pydantic + structlog only; starlette lazily in `web.py`) owns the READ path: `THEMES` (6 curated dark palettes — single source of truth for portal CSS variables AND email colors, `on_accent` slot replacing the inlined button-text hex), `LOGO_MARKS`, `BrandingSettings`, and `current()` with the `bss_cockpit.config` mtime-cache/last-good contract minus bootstrapping and minus crash-on-absence (mount-less containers get defaults + `BSS_BRAND_*` env overrides). ALL writes stay in `bss_cockpit.config` (`write_branding_settings` via tomlkit comment-preserving round-trip + whole-document validation; `write_branding_logo` magic-byte gated PNG/JPEG/WebP ≤256KB fixed filenames, never SVG). Primary UI = cockpit `/settings/branding` (swatch picker, mark picker, first multipart upload in the repo, HTMX live preview via scoped inline CSS-var overrides); `bss branding` CLI verbs + REPL banner follow. Version demoted to a `bss-cli vX.Y.Z` footer/footnote — product attribution, never rebranded. Chat greeting unified: `BSS_OPERATOR_NAME` now an explicit override over `[branding].brand_name`.
**Alternatives:** (a) importing `bss-cockpit` everywhere — drags the Conversation store into `bss-portal-auth` and the subscription service, inverts the dependency graph; (b) home in `bss-models` — drags sqlalchemy/alembic into leaf auth code; (c) env-only — kills hot-reload and the cockpit UI write path; (d) DB table — queryable/tenant-ready but the subscription service reading a cockpit-owned schema bends schema-per-service, and logo bytes still need a file story; revisit if branding ever becomes per-tenant.
**Consequences:** One palette definition drives portals, emails, REPL and the preview card; three new doctrine-check guards (no hex in email.py, no hardcoded brand in templates, settings.toml stays behind the two sanctioned modules). Compose mounts `.bss-cli/` read-only into portal-self-serve + subscription (misconfig visible via the `branding.dir_resolved` startup log; absent mount degrades to phosphor defaults). The CLAUDE.md v0.13 settings.toml line is amended (Phase 0, operator-approved in the v1.8 plan). Light themes deferred — component CSS assumes dark backgrounds.

## 2026-07-19 — 2.0 Phase 8 — Alembic frozen; sqlx baseline via `bss admin migrate`
**Context:** The all-Rust stack still had Python own the schema — `make migrate` = `alembic upgrade head` over `packages/bss-models/alembic` (32 migrations, one shared tree, `public.alembic_version`). Phase 8 retires the Python runtime, so schema creation needs a Rust-native path that doesn't depend on Alembic/greenlet/uv.
**Decision:** Freeze the Alembic tree and capture its end-state as a single sqlx migration `rust/migrations/0001_baseline.sql` (`pg_dump --schema-only --no-owner --no-privileges` on PG16; `alembic_version` excluded; the psql `\restrict`/`\unrestrict` meta-commands and the `set_config('search_path','')` reset stripped so sqlx — which runs SQL, not psql, and needs its own `_sqlx_migrations` ledger visible — applies it cleanly). The sqlx `Migrator` lives in `bss-db` (`migrate` module; workspace sqlx gains `migrate` + `macros`), driven by a new **single-runner** CLI verb `bss admin migrate` over `BSS_DB_URL` — mirroring the one `alembic upgrade head`, NOT per-service-at-startup. Fresh installs apply the baseline; existing (Alembic-created) databases run `bss admin migrate --baseline` once to stamp `0001` as applied without re-running it. Future schema changes are `000N_*.sql` siblings. Runbook: `docs/runbooks/rust-schema-baseline.md`; Makefile `rust-migrate` target added.
**Alternatives:** (a) per-service migrators at startup — rejected: 9-way races on a shared schema, and each service would carry the whole DDL; the single runner matches the Python model. (b) plain `psql -f baseline.sql` via a Makefile target — rejected: no applied-state tracking and no in-binary path for future incremental migrations (`_sqlx_migrations` gives both). (c) keep Alembic and shell out from Rust — rejected: drags Python/uv/greenlet back into the runtime, the exact thing Phase 8 retires. (d) make the baseline idempotent (`CREATE … IF NOT EXISTS` everywhere) so one command covers fresh+existing — rejected: pg_dump can't emit that for constraints/indexes without fragile hand-editing; the `--baseline` stamp is a clean one-time cutover.
**Consequences:** The Rust stack creates its own schema (pgvector required — the `knowledge` schema uses the `vector` type; the baseline runs `CREATE EXTENSION IF NOT EXISTS vector`). Validated end-to-end against a throwaway `pgvector/pgvector:pg16` DB: fresh apply → 16 namespaces + 64 tables + ledger; idempotent re-run no-op; existing-without-`--baseline` fails cleanly; `--baseline` stamp → subsequent no-op. Editing an applied migration changes its checksum (sqlx refuses) — add a new file instead. The Alembic tree stays in the archived Python repo for history + any lingering Python-stack DB.

## 2026-07-19 — 2.0 Cutover — Rust becomes mainline (merge to main; sqlx live-DB stamp; make targets re-pointed)
**Context:** Phase 8 items 1–6 done and the all-Rust stack proven (motto-#6 re-measure: RAM ~14.7× smaller, cold-start ~2×, p99 under budget). The operator chose to replace the plan's fixed 14-day soak with an open-ended "test until it's actually ready" gate, and asked to land the cutover now while holding the release.
**Decision:** (1) Merged `2.0` → `main` with a single `--no-ff` merge commit (`c187f96`) — Rust is now the mainline (169 commits; the Python tree stays in-tree as the reproducible oracle). (2) Stamped the live `tech-vm` DB with the sqlx baseline (`bss admin migrate --baseline`) so it is sqlx-managed going forward (`_sqlx_migrations` version=1; Alembic frozen). (3) Re-pointed the canonical `make test/fmt/lint/migrate/seed/doctrine-check` to the Rust workspace; the Python bodies are preserved verbatim as `py-*` (the oracle) — nothing deleted. **Explicitly HELD until the operator calls the release:** archiving/removing the Python source, and `git tag -a v2.0.0`. The Python oracle stays runnable through the operator's testing so we can golden-diff when bugs surface.
**Alternatives:** (a) hold the whole cutover behind the fixed 14-day soak — rejected by the operator as artificial for a solo reference build; the open-ended test gate is more honest. (b) fast-forward the merge — rejected: a `--no-ff` merge commit marks the Rust landing as one trivially-revertible unit. (c) delete the Python tree now — rejected: it's the golden-diff oracle during testing; it retires with the release tag. (d) rename make targets destructively (drop the Python bodies) — rejected: `py-*` preserves them at zero cost until the archive.
**Consequences:** `main` builds/runs all-Rust; `docker-compose.rust.yml` is the deployed stack. Bug fixes now land on `main`. The remaining release acts (Python archive + `v2.0.0` tag) are a short, well-specified batch (checklist in `phases/2.0/07-SOAK.md`) run when the operator is satisfied. Revert path if needed: `git revert -m 1 c187f96`.

## 2026-07-19 — 2.0 Flip — Rust promoted to the repo root; Python → python-legacy/
**Context:** During the migration the Rust workspace lived in `rust/` (Decision D7) so Python (the oracle) + Rust could coexist — they can't share the root because `services/`, `portals/`, `cli/` collide. After the 2.0→main cutover the operator wanted the repo to reflect reality (Rust is mainline) rather than leaving the production code "hidden" one level down. The flip is one coupled operation: promoting Rust requires moving Python aside first.
**Decision:** Moved the Python tree (`cli/ orchestrator/ packages/ portals/ services/ soak/ tests/` + `pyproject.toml`, `uv.lock`, the two top-level scripts) into `python-legacy/` (kept, still the runnable oracle for golden-diffs), and promoted `rust/*` to the repo root (`crates/ services/ portals/ cli/ conformance/ migrations/ Cargo.toml Cargo.lock rust-toolchain.toml`; `rust/README.md`→`docs/RUST-WORKSPACE.md`). **Decoupled the portal assets:** the Rust portals reused Python's Jinja templates + static verbatim; those trees are now copied into `portals/*/assets` + `crates/bss-portal-ui/assets` so the Rust images own them and `.dockerignore` can exclude `python-legacy/` cleanly (no fragile negation). Path fixes: the Rust service Dockerfiles are unchanged (context `rust`→`.` via the overlay + `.dockerignore`); the 2 portal Dockerfiles rewritten (WORKDIR/COPY/binary path + local asset paths); `docker-compose.rust.yml` service context `rust`→`.` + portal dockerfile paths; base `docker-compose.yml` Python build context `.`→`python-legacy`; `scripts/rust_doctrine_check.sh` `RUST=.`; the Makefile (`rust-*` drop `cd rust`, `py-*`/legacy targets `cd python-legacy`, `knowledge-reindex`/`scenarios`→Rust, `reset-db` also drops `_sqlx_migrations`); `.github/workflows/rust.yml` (root paths + doctrine-check step); `docker-compose.e2e.yml` fixtures path. The build cache moved `rust/target`→`./target`.
**Alternatives:** (a) keep `rust/` as a permanent subtree — rejected: the operator wanted Rust visibly canonical. (b) portal Dockerfiles keep referencing `python-legacy/` assets with a `.dockerignore` negation to re-include them — rejected as fragile (BuildKit negation gotchas); decoupling into the Rust dirs is robust and the correct end-state. (c) delete Python now — rejected: it stays as the golden-diff oracle until the release.
**Consequences:** The repo root IS the Rust workspace; `cargo`/`bss` run straight from root. Validated: `cargo check` (8.8s) + `make doctrine-check` + `cargo test -p bss-db` (sqlx `migrate!("../../migrations")` resolves) green from root; catalog + self-serve images build from the root context and catalog runs (/health + TMF read 200). `python-legacy/` is a duplicated-asset artifact of the coexistence period; it (and the asset dupes) retire at the `v2.0.0` release. Revert: this flip is one commit; `git revert` it (or `git mv` back). The 2.0→main merge commit `c187f96` still reverts the whole Rust landing.

## 2026-07-21 — v2.0.0 — Release cut; Python oracle archived; soak gate consciously overridden
**Context:** Phase 8's cutover (`c187f96`) landed Rust on `main` and held two acts for the release: archive the Python oracle and `git tag -a v2.0.0`. The documented gate (`phases/2.0/07-SOAK.md`) was a 14-day don't-regress soak nominally closing 2026-08-02. Post-cutover bug-fix work (MQ auto-resubscribe `dc63fda`, Stripe add-card, MiniJinja flip artifacts, and a session of `.env`/branding/banner/transcript fixes) kept resetting the soak clock. The operator judged the stack stable and elected to cut `v2.0.0` now rather than wait out the soak window.
**Decision:** Cut `v2.0.0` on 2026-07-21. (1) `BSS_RELEASE` 1.8.1 → 2.0.0 (`bss-models`; the pinned-to-oracle test becomes `release_is_current`). (2) **Archived the Python oracle, not deleted** (per the original strategy D-decision): annotated git tag `python-oracle-final` at the pre-removal HEAD + a 5.7 MB tarball in `~/archives/`, a recovery pointer at `docs/PYTHON-ORACLE.md`, then `git rm -r python-legacy/`. (3) Rewired every in-tree reference: base `docker-compose.yml` build contexts `python-legacy` → `.` (the Rust overlay already overrode them); removed the retired Python dev-loop Makefile targets (`py-*`, `python-check`, `check-clock`, `lint-types`) and the Python `e2e*` targets + `docker-compose.e2e.yml`; updated README/HANDBOOK/CLAUDE.md (two Phase-0 amendment lines). (4) Ran the full gate green (fmt, clippy -D warnings, `cargo test` workspace, 14 doctrine guards, hero suite) then tagged.
**Alternatives:** (a) Wait for the 2026-08-02 soak close — rejected by the operator as artificial for a solo reference build given the open-ended "test until ready" gate adopted at cutover; the override is recorded here rather than pretended away. (b) Delete `python-legacy/` outright — rejected; the strategy always said archive (tag + pointer), and the oracle stays golden-diff-recoverable. (c) Port the residual Python-only tooling first — deferred; see follow-up.
**Consequences:** `main` is all-Rust with no in-tree Python; `git tag v2.0.0`. Alembic fully retired (sqlx baseline under `migrations/`). **Known regression / follow-up:** the demo seed (`bss_seed.demo` → `make seed-demo`/`seed-demo-reset`/`loyalty-reset`/`demo-restore`) was Python-only and is NOT yet ported — intended Rust replacement is `bss admin seed-demo`; until then seed demo data via the archived oracle or by hand. The Python Playwright e2e suite (`make e2e`) retired with no Rust equivalent beyond `make scenarios-hero`. Soak was cut ~12 days short of the nominal window — if a latent regression surfaces, the recovery oracle is one `git worktree add … python-oracle-final` away.

## 2026-07-21 — v2.0.0 follow-up — demo seed ported to `bss admin seed-demo`
**Context:** The v2.0.0 cut (above) flagged one known regression: the demo seed (`bss_seed.demo` → `make seed-demo`/`seed-demo-reset`/`loyalty-reset`/`demo-restore`) was Python-only and retired with the oracle, with `bss admin seed-demo` named as the intended Rust replacement. Cutting the release before porting it left the demo/loyalty convenience flow (which the operator actively uses) without a Rust path.
**Decision:** Ported `seed` + `reset` to a new `bss admin seed-demo [--reset]` (`cli/src/commands/admin_seed_demo.rs`), re-exposed as `make seed-demo` / `seed-demo-reset`. Faithful to the Python: 3 demo customers (stable key = `*.demo@bss-cli.local` email), 2 promotions (`PROMO_DEMO_WELCOME` public 10%/multi-3, `PROMO_DEMO_VIP` targeted 20%/single), Alice+Bob assigned to the VIP. Promotions go through the catalog HTTP client so the loyalty saga + CRM eager-sync fire server-side (loyalty in lockstep); a `catalog.promotion.loyalty_not_configured` refusal degrades to BSS-only mode. Existence checks + the surgical reset deletes use `BSS_DB_URL` directly (all keys are `text`), exactly as the Python did. DB errors funnel through `ClientError::Transport` so `run_safely_code`'s existing error/`POLICY_VIOLATION` mapping is reused. Validated live end-to-end: seed → idempotent re-run (all skip) → `promo show` (state=active, OD linked) → `--reset` → DB verified clean (0 demo customers / promos / eligibility rows).
**Alternatives:** (a) Port `loyalty-wipe` + `demo-restore` too — rejected: `loyalty-wipe` truncates loyalty-cli's OWN schema and re-stamps ITS alembic (via `docker inspect loyalty-http`); BSS reaching into another service's DB/migrations is a layering violation. loyalty-cli owns its reset; `demo-restore` (reset-db + loyalty-wipe + seed-demo) is therefore not reconstituted in-tree. (b) Raw-DB seed (bypass the services) — rejected: would skip the loyalty saga + CRM eager-sync, defeating the "in lockstep" point. (c) A loyalty admin client for the best-effort loyalty-side customer delete in reset — dropped: the unassign already revokes the offers; stale demo customer rows in loyalty are harmless and idempotent.
**Consequences:** The v2.0.0 "known regression" is closed except `loyalty-wipe`/`demo-restore` (documented as out, PYTHON-ORACLE.md + HANDBOOK). CLAUDE.md v2.0.0 amendment updated; soak follow-up checked.

## 2026-07-22 — v2.1 — Phase 0 amendment: catalog management joins the LLM surface, gated by `/confirm`
**Context:** The operator reported catalog management (offering CRUD) as unavailable in the REPL and the CSR cockpit chat. The v1.6.1 browser screens already carried the full CRUD (add offering / add price / set window / retire, all `admin_*` client calls behind the two-step UI confirm), and the CLI carried `bss admin catalog *`. The gap was the *conversational* surface: `catalog.add_offering`, `catalog.add_price`, and `catalog.window_offering` were registered and in the `operator_cockpit` profile but sat in `_LLM_HIDDEN_TOOLS` since v0.7, so `surface()` filtered them out and the model never saw them. `catalog.retire_offering` had no tool at all — CLI + screen only. Notably the cockpit's `DESTRUCTIVE_PREFIXES` already listed the three, i.e. the staging machinery was in place for a surface that didn't exist.
**Decision:** Move the three out of `LLM_HIDDEN_TOOLS` and into `safety::DESTRUCTIVE_TOOLS`; add a fourth tool, `catalog.retire_offering`, wrapping the existing `CatalogClient::admin_retire_offering`, to `OPERATOR_COCKPIT` + both destructive lists. **`/confirm` replaces invisibility as the gate** — a strictly stronger one, because the operator reads the staged args before the write lands, where hiding just meant the capability didn't exist. Multi-turn elicitation is enforced in two places, because the orchestrator emits a permissive function schema (`additionalProperties: true`) and the description prose is therefore the *only* arg contract the model receives: (1) each of the four descriptions was rewritten as an explicit elicitation contract — required vs. optional fields, "ask in one plain-prose question and stop", "never derive a plan id from the name / price by analogy / assume allowances typical for this tier"; (2) `tools::require_fields` gates server-side, returning a `MISSING_REQUIRED_FIELDS` observation that names *every* absent field at once so the model asks once instead of probing. `add_offering` additionally rejects a plan granting no allowance (pass `0` for a deliberate omission); `window_offering` rejects both bounds absent, treating an explicit `null` as a deliberate clear (key-presence check, not value check). An elicitation bounce must not count as a tool failure: `execute_tool` now reports it with `is_error = false` (new `ToolError::is_elicitation`, keyed on the `MISSING_REQUIRED_FIELDS` const), so it stays out of the 3-strike bail. **This was found by the new loop test, not by reading the code** — the first cut assumed "not in `FAILURE_MARKERS`" was sufficient, but `is_failure_result` is `is_error || markers` and *every* `Err` sets `is_error`, so three clarifying questions in one turn bailed the turn with a "couldn't recover" panel. The identical-call stuck bail still catches a model looping on the same incomplete args, which is the case that genuinely warrants a bail.
**Alternatives:** (a) Leave them hidden and point the operator at the CSR screens / CLI — rejected, that is the reported gap, and doctrine (motto #4) says every capability is a tool the LLM can call. (b) Expose them ungated — rejected: no catalog write has a one-call undo (`add_price` can only be superseded, never removed), a bad window silently unsells a live plan, and `list_active_offerings` feeds order creation, so a bad row is sellable the instant it lands. (c) Put the elicitation rules in `cockpit_invariants.txt` — rejected for now: that file is pinned byte-for-byte by the Python-parity golden (`prompt_golden.rs`), and the existing anti-mimicry section already sanctions "ask in clean prose when you genuinely need more info", so the tool descriptions are both sufficient and the correct altitude for an *arg* contract. (d) Enforce completeness in the catalog service's policy layer — rejected: "the operator was actually asked" is a property of the conversation, not a domain invariant; the CLI and CSR screens legitimately call the same client with the same partial payloads.
**Consequences:** The cockpit can now run catalog CRUD conversationally, each verb staged as a propose the operator authorises with `/confirm` — same flow as `subscription.terminate`. The three descriptions **deliberately diverge from the retired Python oracle** (they described tools no model ever read), so they left the `tool_descriptions_match_python_oracle` parity list; the contract they must now hold is pinned by the new `catalog_writes_carry_the_elicitation_contract` test (profile membership, non-hidden, destructive, description shape, and the computed surface in both directions). Rust is the source of truth for these four descriptions from here. `subscription.migrate_to_new_price` stays hidden (regulatory-notice obligation the model can't discharge) and `usage.simulate` stays hidden (burns real allowance). Not exposed to `customer_self_serve`, asserted both ways. TOOL_SURFACE.md rows 117-119 amended + a retire row added under the operator's explicit Phase 0 authorisation.

## 2026-07-23 — v2.1.1 — Dangling identity→customer link self-heals; one narrow carve-out to the 1:1 link rule
**Context:** The operator hit "Something went wrong" creating an order for `chiamck+001@icloud.com` on number 90000001. Investigation: `portal_auth.identity` held `customer_id` values for three `chiamck+00N` identities whose `crm.customer` rows no longer existed (only 2 customers survive, both from 07-21; the three `customer.created` audit events from 07-20 are still in `audit.domain_event`). Signup's returning-customer branch calls `crm.get_kyc_status(cid)`, which returns 422 `POLICY_VIOLATION` / `customer.kyc.not_found`; the rule had no entry in `RULE_MESSAGES`, so it rendered `GENERIC_FALLBACK`. **Not a signup regression** — no signup-path change caused it. **[CORRECTED 2026-07-23, same day: the original entry claimed no in-tree code deletes arbitrary customers and that the rows went "out-of-band". Both were wrong — see the 2026-07-23 hero-suite entry below. `make scenarios-hero` truncated them.]** The defect is that the portal had no handling for the state: the identity was bricked permanently, and because reserve-at-pick runs *before* the customer lookup, every retry burned another MSISDN hold (`OO-3ea9832…` was still holding 90000001).
**Decision:** Signup treats `customer.kyc.not_found` on the linked customer as a *stale link*, not a dead end: log `portal.signup.identity_link_dangling` at ERROR, write a `portal_action` row (`signup.create_customer.identity_link_dangling`), drop the link, and fall through to the existing new-customer path, re-binding via a new `bss_portal_auth::relink_orphaned_to_customer`. That helper is **the one exception** to `link_to_customer`'s "links are 1:1 and not reassignable from this surface" rule, and it is kept narrow by construction: the UPDATE is a compare-and-swap on the *ghost* customer id, so the only reachable transition is ghost → new customer. An identity bound to a live customer cannot be moved through it — the CAS misses and returns `AlreadyLinked` naming the real holder — so it opens no account-takeover surface even though the caller picks `new_customer_id`. The new-customer mock-PAN guard is re-applied on the fall-through (it is skipped upstream because the identity still looks like a returning customer at that point). `customer.kyc.not_found` also gained real copy, for the surfaces that read a linked customer *without* repairing it (CSR `customers.rs`, the `customer.get_kyc_status` tool).
**Alternatives:** (a) Fail loudly with honest copy and repair links by hand — rejected by the operator; it leaves the customer stuck on a self-serve surface for a fault they cannot see or act on. (b) Self-heal behind a `BSS_PORTAL_RELINK_ORPHANED` flag — rejected as two paths to test and a doctrine surface of its own for a state that is never legitimate. (c) Make `link_to_customer` generally reassignable — rejected outright: that *is* the takeover surface the 1:1 rule exists to prevent. (d) A DB foreign key from `portal_auth.identity.customer_id` to `crm.customer.id` — rejected, it would cross the schema-per-service boundary that exists so services can later split to separate Postgres instances.
**Consequences:** A signup against a ghost link now completes unaided and leaves a loud ERROR log + audit row, so the underlying data fault is still visible rather than papered over. The CAS semantics are pinned by three new cases in the (live, `#[ignore]`) `audit_link_live` round-trip — CAS miss refuses *and* leaves the link untouched, CAS hit repairs, unknown identity errors rather than silently no-opping. The self-heal keys off CRM's exact rule string across a service boundary; a rename in `services/crm/src/service.rs` would silently restore the original defect, so `dangling_link_rule_is_pinned_and_has_copy` pins the literal and its copy-map entry in CI. **[CORRECTED same day:** the cause is no longer unexplained — `make scenarios-hero` truncated the rows. See the hero-suite entry below.**]**

## 2026-07-23 — v2.1.2 — The hero suite wipes ALL customer data, and the wipe was half-done (left logins behind)
**Context:** Correction to the entry above, found because the operator rejected "removed out-of-band" — they had not touched those customers. They were right. 18 of the 19 hero scenarios declare `setup.reset_operational_data: true`, which fans `admin.reset_operational_data` out to 7 services; CRM's `ResetPlan` (`services/crm/src/lib.rs`) `TRUNCATE`s **every row** of `customer`, `party`, `contact_medium`, `individual`, `interaction`, `customer_identity`, `case`, `ticket` — not scenario-scoped, no "only rows this run made" filter, gated only by `BSS_ALLOW_ADMIN_RESET=true` which is set in the operator's `.env`. Proof: `make scenarios-hero` ran 02:12:28Z→~02:37Z on 2026-07-21 during v2.0.0 release verification; 16 `admin.operational_data_reset` markers landed that day; the last CRM one at 02:37:38.853Z, 0.4s before `trace-4bed5910@bss-cli.local` was created by the final scenario. **Why the first investigation missed it:** (1) the TRUNCATE SQL is `format!`-built from a table-name list, so grepping `DELETE FROM crm.customer` / `TRUNCATE crm.customer` finds nothing — searching for the *literal* instead of the *capability* was the actual error; (2) `audit.domain_event` is deliberately spared, so `customer.created` events survive for customers that no longer exist, which reads exactly like an out-of-band delete; (3) having exonerated `seed-demo --reset` and `make reset-db`, the investigation concluded "therefore nothing in-tree" rather than "therefore some other in-tree path", and stopped one query short — `WHERE event_type='admin.operational_data_reset'` would have ended it immediately.
